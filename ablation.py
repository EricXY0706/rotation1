...
class binary_classification_model(nn.Module):
    ...
    def forward(self, x, seq_length, device):
        ...
        representations_attn = self.projection(x) # [b, s, e] -> [b, s, p]
        representations_proj = rearrange(representations_attn, 'b s p -> b p s')
        ...
        representations_attn = rearrange(representations_conv, 'b h s l -> b s (l h)') # [b, s, p]
        for i, l in enumerate(seq_length):
...
def model(ESM_model_name, result_name, parameters_dict):
    ...
    fpr, tpr, thresholds = roc_curve(target_test.cpu().numpy()[:, 0], y_prob.cpu().detach().numpy()[:, 0], pos_label=1)
    ...
    metrics(TP, FP, TN, FN)
    # Record
    torch.save(binary_model.state_dict(), rf'/home/xuyi/rotation1/binary_model_results/model/b_{result_name}_wo_conv_attn.pth')
    ...
    plt.legend()
    plt.savefig(rf'/home/xuyi/rotation1/binary_model_results/ROC/{result_name}_wo_conv_attn.tiff', dpi=300, bbox_inches='tight')
    plt.show()
