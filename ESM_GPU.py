...
def model(ESM_model_name, result_name, parameters_dict):
	...
	token_representations = torch.load(rf'/home/xuyi/rotation1/ESM_embeddings/tokens/tokens_{ESM_model_name}_0109_{positive_dataset}_{negative_dataset}.pt').to(device)
	sequence_length = torch.load(rf'/home/xuyi/rotation1/ESM_embeddings/seq_length/seq_length_{ESM_model_name}_0109_{positive_dataset}_{negative_dataset}.pt').to(device)
	...
	metrics(TP, FP, TN, FN)
	# Record
	# torch.save(binary_model.state_dict(), rf'/home/xuyi/rotation1/final_ablation/wo_conv_attn/b_{result_name}_0109_{positive_dataset}_{negative_dataset}.pth')
	# np.save(rf'/home/xuyi/rotation1/ESM-tSNE/binary_y.npy', target_test.cpu().numpy()[:, 0])
	# np.save(rf'/home/xuyi/rotation1/ESM-tSNE/binary_test_attention.npy', out.cpu().detach().numpy())
	plt.figure()
	...
	plt.legend()
	# plt.savefig(rf'/home/xuyi/rotation1/binary_model_results/ROC/{result_name}_0109_{positive_dataset}_{negative_dataset}.pdf', dpi=300, bbox_inches='tight')
if __name__ == '__main__':
	params = {"positive_dataset": '20*90',
